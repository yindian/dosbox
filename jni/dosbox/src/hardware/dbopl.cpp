...
void InitTables( void ) {
	...
	for ( int i = 0; i < 256; i++ ) {
		...
		ExpTable[i] = (int)( 0.5 + ( powf(2.0, ( 255 - i) * ( 1.0 /256 ) )-1) * 1024 );
	...
	for ( int i = 0; i < 512; i++ ) {
		SinTable[i] = (Bit16s)( 0.5 - log10f( sinf( (i + 0.5) * (PI / 512.0) ) ) / log10f(2.0)*256 );
	}
#endif
#if ( DBOPL_WAVE == WAVE_TABLEMUL )
	...
	for ( int i = 0; i < 384; i++ ) {
		...
		double val = ( 0.5 + ( powf(2.0f, -1.0f + ( 255 - s) * ( 1.0f /256 ) )) * ( 1 << MUL_SH ));
		MulTable[i] = (Bit16u)(val);
	}

	//Sine Wave Base
	for ( int i = 0; i < 512; i++ ) {
		WaveTable[ 0x0200 + i ] = (Bit16s)(sinf( (i + 0.5) * (PI / 512.0f) ) * 4084);
		WaveTable[ 0x0000 + i ] = -WaveTable[ 0x200 + i ];
	}
	//Exponential wave
	for ( int i = 0; i < 256; i++ ) {
		WaveTable[ 0x700 + i ] = (Bit16s)( 0.5 + ( powf(2.0, -1.0f + ( 255 - i * 8) * ( 1.0f /256 ) ) ) * 4085 );
	...
	//Sine Wave Base
	for ( int i = 0; i < 512; i++ ) {
		WaveTable[ 0x0200 + i ] = (Bit16s)( 0.5 - log10f( sinf( (i + 0.5) * (PI / 512.0f) ) ) / log10f(2.0f)*256 );
